#!/bin/bash
#SBATCH --job-name=ft-pascal-finetune
#SBATCH --nodes=1
#SBATCH --ntasks=1
#SBATCH --gres=gpu:1
...
RUN_DIR=/scratch/work/saritak1/segmentation/output_finetune_pascal/${CKPT_FOLDER}
mkdir -p "$RUN_DIR"

# ---- WandB credentials ------------------------------------------------
export WANDB_API_KEY=$(cat ~/.wandb_key)       # set in ~/.bashrc or via sbatch --export
export WANDB_PROJECT="pascal-finetune"
...
srun python main.py fit \
  -c configs/pascal_voc_mask2former_semantic.yaml \
